...
```{r}
# download abbreviations table
abbreviationsWebPage <- "https://en.wikipedia.org/wiki/List_of_U.S._state_abbreviations"
abbreviationsTables <-
  abbreviationsWebPage %>%
  read_html() %>%
  html_nodes(css = "table") %>%
  html_table(fill = TRUE)

abbreviationTableOriginal <- abbreviationsTables[[1]]

abbreviationTable <- read.table(abbreviationTableOriginal, skip = 9)
```
